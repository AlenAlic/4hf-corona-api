...
from .email import send_password_reset_email, send_activation_email, send_password_changed_email
...
class AuthAPICreate(Resource):
    ...
    @api.response(400, "Email address taken")
    def post(self):
        """Create user"""
...
class AuthAPIActivate(Resource):
    ...
    def post(self, token):
        ...
        u = User.query.filter(User.activation_code == token).first()
        if u is not None:
